...
export const buildSearchDataFromMenu = (
  menus: MenuItem[]
): SearchData[] => {
  ...
  const walk = (items: MenuItem[], parentSection?: string) => {
    for (const item of items) {
      const section = parentSection ?? item.label
...
const NavSearch = ({ dictionary, permission }: NavbarProps) => {


  // Transform the data to group items by their sections
  const transformedData = buildSearchDataFromMenu(MenuFilterPermission(permission, dictionary)).reduce((acc: Section[], item) => {
        ...
        <CommandList>
          {searchValue ? (
            limitedData.length > 0 ? (
              ...
              <CommandEmpty>
                <NoResult permission={permission} dictionary={dictionary} searchValue={searchValue} setOpen={setOpen} />
              </CommandEmpty>
            )
          ) : (
            <DefaultSuggestions permission={permission} dictionary={dictionary} setOpen={setOpen} />
          )}
        </CommandList>
        <CommandFooter />
      </CommandDialog>
